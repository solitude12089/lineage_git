...
class CrossController extends Controller
{
	public function getIndex()
    {
    	...
    	$cross_treasures = \App\models\CrossTreasure::with('details')
                                ->with('owner_name')
                                ->with('details.user')
                                ->where('status',"1")
    	...
    	foreach ($cross_treasures as $key => $value) {
    		$value->is_join = false;
    		...
    		foreach ($value->details as $k => $v) {
                ...
                $name = $v->user->customer->username.'_'.$v->user->name;
			 	if($k==0){
			 		$value->d_string=$value->d_string.$name;
			 	}
			 	else{
			 		$value->d_string=$value->d_string.','.$name;
			 	}
    	...
    	return view('cross.index',['treasures'=>$cross_treasures,'boss_list'=>$boss_list]);
    }
    ...
    public function getInfo($treasure_id)
    {
    	...
    	$user=Auth::user();
    	$boss_list = \App\models\Boss::all()->pluck('name','id');
    	$treasure = \App\models\CrossTreasure::with('user')
    							->with('owner_name')
    							->with('details')
    							->with('details.user')
    							->with('attachments')
    	...
    	return view('cross.info',['user'=>$user,'treasure'=>$treasure,'boss_list'=>$boss_list,'now'=>$now]);
    }
    ...
    public function getJoin($treasure_id)
    {
        ...
        $details = \App\models\CrossTreasure_detail::where('treasure_id',$treasure_id)
    ...
    public function getUnjoin($treasure_id)
    {
        ...
        $user=Auth::user();
        $detail = \App\models\CrossTreasure_detail::where('treasure_id',$treasure_id)
                                                ->where('user_id' ,$user->email)
                                                ->where('type',2)
                                                ->first();
    ...
    public function getAdd()
    {
        ...
        $crossdb = $user->customer->crossdb;
        $customer_ids = \App\models\Customer::where('crossdb',$crossdb)->get()->pluck('id');
        
    	$user_list = \App\User::with('customer')
        ...
        foreach ($user_list as $key => $value) {
            $bygroup[$value->customer->username][]=$value;
        }
       
    	return view('cross.add',['boss_list'=>$boss_list,'user_list'=>$user_list,'bygroup'=>$bygroup]);
    }
    ...
    public function getTestA(){
        $user = Auth::user();
        $sql = "SELECT CONCAT(c.username,'_',u.name) AS nn , u.email
                FROM users as u
                JOIN customer as c on u.customer_id = c.id
                where u.status=1 and c.crossdb = '".$user->customer->crossdb."'";

        $table = DB::connection('lineagepublic')->select($sql);
        foreach ($table as $key => $row) {
            $rt_data[$row->email] = $row->nn;
        }
        ...
        $user_map = \App\User::join('customer','customer_id','=','customer.id')
            ->whereIn('customer_id',$customer_ids)
            ->where('status',1)
            ->get();
        dd($user_map[0]);
 	...
 	public function postAdd(Request $request)
    {
       ...
       // dd($data['_item']);
        ...
        foreach ($data['_item'] as $item_key => $item_value) {
            ...
            $first_note= \App\models\CrossTreasure::where('no','like',"R".$date.'%')
                                        ->orderBy('no','desc')
                                        ->first();
            if($first_note)
            {
            ...
            $new_note=new \App\models\CrossTreasure;
            $new_note->no="R".$_no;
            ...
            if(isset($data['_members'])){
            	foreach ($data['_members'] as $k => $v) {
            		$dtl=new \App\models\CrossTreasure_detail;
...
//             $msg=$user->customer->username.'_'.$user->name." 建立了寶物申報單 ".$new_note->no." 
// 日期：".$new_note->day." 
// Boss：".$boss_list[$new_note->boss_id]." 
// 寶物：".preg_replace('/[\d]/','*',$new_note->item)." 
// 持有人：".$user_map[$new_note->owner]." 
// 參與人：".$msg_members." 
// http://www.lineagebank.tw/cross/info/".$new_note->id;
