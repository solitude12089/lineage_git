...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ajax-modal" href="{{url('cross/add')}}">新增</button>
        </div>
                    ...
                    <form id='nextform' class="form-horizontal" action="{{url('note/create-view')}}" method="post">
                        ...
                        <table id="table" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><a href="{{url('cross/info').'/'.$value->id}}">{{$value->no}}</a></td>
                                    <td class="rwd_hide">{{$boss_list[$value->boss_id]}}</td>
                                    <td>{{$value->day}}</td>
                                    <td>{{$value->item}}</td>
                                    <td class="rwd_hide">{{$value->owner_name->customer->username.'_'.$value->owner_name->name}}</td>
                                    <td class="rwd_hide">
                                    ...
                                    <td class="rwd_hide">{{$value->user->customer->username.'_'.$value->user->name}}</td>
