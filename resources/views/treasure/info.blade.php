    ...
    <div class="row">
        <div class="col-md-12">
            ...
            @if(date('Y-m-d H:i:s',strtotime($treasure->created_at . '+1 days'))>$now)
                @if(!$treasure->is_join)
                <a class="btn btn-success" href="{{url('treasure/join').'/'.$treasure->id}}">申請補登</a>
                @endif
                @if($treasure->is_complement)
                <a class="btn btn-danger" href="{{url('treasure/unjoin').'/'.$treasure->id}}">取消補登</a>
                @endif
            @else
                @if($treasure->status=='1' && $treasure->owner == $user->email)
                    <a class="btn btn-info" href="{{url('offset/require').'/'.$treasure->id}}">扣薪入帳</a>
                @endif
            @endif
            @if(strpos(Auth::user()->role_id, '5') !== false)
                <a class="btn btn-info" href="{{url('treasure/edit').'/'.$treasure->id}}">編輯</a>
                <a class="btn btn-danger" href="{{url('treasure/delete').'/'.$treasure->id}}">刪除單據</a>
            @endif
